...
import { bpmRetryEligibilityTrackWhere } from "@/lib/bpm";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const targetWhere = trackIds?.length
      ? { id: { in: trackIds } }
      : isBpmHealthFilter(filter) ? bpmHealthFilterWhere(filter) : { id: "__invalid__" };
    ...
    const where = {
      AND: [
        ...
        targetWhere,
        bpmRetryEligibilityTrackWhere({ force, providerMode }),
      ],
    };
