...
2. Duplicate `.env.example` to `.env` and fill in your API keys (Plex Client Identifier, Discogs/Last.fm/Spotify as needed).
   - Local BPM analysis defaults to `LOCAL_BPM_ANALYZER=auto`, which uses Essentia's BPM-focused `RhythmExtractor2013` when available and logs when it falls back to Aubio. Samples are written atomically and must pass size, audio-stream, and duration checks through ffprobe before analysis. Use `LOCAL_BPM_ANALYSIS_SCOPE=whole_track` for slower full-track BPM analysis, or keep the default `windows` mode for multi-window analysis. Local Essentia audio-feature fallback also supports `LOCAL_AUDIO_FEATURES_SCOPE=whole_track`; the default `windows` mode analyzes 30s-90s, the middle window, and the last-third window. Automatic initial enrichment does not launch the large local Essentia audio-feature backfill unless `LOCAL_AUDIO_FEATURES_AUTO_BACKFILL=1`; the manual Audio Feature sync can still run it. To avoid Plex transcode failures in Docker, mount your media into the app container and set `PLEX_MEDIA_PATH_HOST` plus `MIXARR_MEDIA_PATH_CONTAINER`, or `MIXARR_PATH_MAPPINGS=/mnt/media:/media,/data:/data`. The mapping rewrites the path Plex reports; the Compose volume mounts the real Docker host folder. For example, if Plex reports `/mnt/Music/...` but the Docker host stores files at `/mnt/plex/Music/...`, use `MIXARR_PATH_MAPPINGS=/mnt/Music:/media` and mount `/mnt/plex/Music:/media:ro`. Set `LOCAL_BPM_REPROCESS_NO_DATA_FAILED=1`, or enable the matching Sync Options checkbox, to retry tracks previously marked as BPM no-data, extraction-failed, or analyzer-failed.
